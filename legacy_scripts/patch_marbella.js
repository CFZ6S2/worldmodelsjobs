...
const wf = JSON.parse(fs.readFileSync('scratch/final_v5_patched_fixed11.json', 'utf8'));
...
if (ibizaRegex.test(cityRaw) || ibizaRegex.test(textRaw)) {
  ...
  // Client 3 (Moved from Madrid to Ibiza eventos)
  if (item.category === 'evento') {
    langs.push({ code: 'ES_CLIENT_IBIZA_3', tg: '6756692518', wa: '34615787912@s.whatsapp.net', title: item.title_es || 'Nuevo Lead', text: item.text_es || item.texto_limpio, tag: 'Remitente' });
  }
}

// MARBELLA
const marbellaRegex = /(marbella|puerto banus|banus)/i;
if ((marbellaRegex.test(cityRaw) || marbellaRegex.test(textRaw)) && item.category === 'evento') {
  langs.push({ code: 'ES_CLIENT_MARBELLA', tg: '1145608699', wa: '34607426796@s.whatsapp.net', title: item.title_es || 'Nuevo Lead', text: item.text_es || item.texto_limpio, tag: 'Remitente' });
}
...
const routingTable = {
  ...
  "ibiza": {
    ...
    targets: [
      ...
      { to: "353830078788@s.whatsapp.net", label: "IBIZA" },
      { to: "34615787912@s.whatsapp.net", label: "IBIZA", categoryFilter: "evento" }
    ]
  },
  "marbella": {
    keywords: ["marbella", "puerto banus", "banus"],
    targets: [
      { to: "34607426796@s.whatsapp.net", label: "MARBELLA", categoryFilter: "evento" }
    ]
  }
};
...
// Aplicar categoryFilter si existe
...
fs.writeFileSync('scratch/final_v5_patched_fixed12.json', JSON.stringify(wf, null, 2));
console.log("scratch/final_v5_patched_fixed12.json written!");
