...
if (monacoRegex.test(cityRaw) || monacoRegex.test(textRaw)) {
  // Monaco -> WA client, TG ID client, TG Group, plus the normal groups above
...
if ((madridRegex.test(cityRaw) || madridRegex.test(textRaw)) && item.category === 'evento') {
  // Madrid -> WA client, TG ID client, TG Group. The client ONLY gets Madrid.
...
// 2. PATCH DYNAMIC ROUTING ENGINE (Remove London, Paris, etc)
...
fs.writeFileSync('scratch/final_v5_patched_fixed3.json', JSON.stringify(wf, null, 2));
console.log("scratch/final_v5_patched_fixed3.json written!");
