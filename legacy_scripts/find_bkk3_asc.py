...
try:
    ...
    c.execute("SELECT executionId, data FROM execution_data WHERE data LIKE '%БАНГКОК%' AND data LIKE '%Требуется привлекательная%' ORDER BY executionId ASC LIMIT 5")
    rows = c.fetchall()
