...
const wf = JSON.parse(fs.readFileSync('scratch/final_v5_patched_fixed13.json', 'utf8'));
...
if ((madridRegex.test(cityRaw) || madridRegex.test(textRaw)) && item.category === 'evento') {
  // Client PT (Brasileña) eliminada por impago
  // Group PT
...
fs.writeFileSync('scratch/final_v5_patched_fixed14.json', JSON.stringify(wf, null, 2));
console.log("scratch/final_v5_patched_fixed14.json written!");
