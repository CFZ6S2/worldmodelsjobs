...
export async function POST(request: Request) {
  try {
    ...
    if (result.error) {
      const status = (result.error === 'SENDER_BANNED' || result.error === 'LEAD_BANNED_CONTENT') ? 403 : (result.error === 'LEAD_EMPTY_OR_TOO_SHORT' ? 400 : 500);
      return NextResponse.json(result, { status, headers: CORS_HEADERS });
    }
    if (result.message && (result.message.includes('rechazado') || result.message.includes('descartado'))) {
      return NextResponse.json(result, { status: result.message.includes('EXCLUSIVAMENTE') ? 403 : 200, headers: CORS_HEADERS });
    }
...
export async function GET() {
  return NextResponse.json({
    status: 'API WorldModels V2 Active',
