...
def patch():
    ...
    # ULTIMATE ROBUST METADATA EXTRACTION
...
function getText(obj) {
    ...
    // 1. Mirar caption (para imágenes)
    if (obj.caption) return String(obj.caption);
    
    // 2. Mirar estructura de texto de Whapi/WhatsApp
    if (obj.text) {
...
return [{ json: {
  ...
  looksValuable: textBody.trim().length > 5
    ...
    conn.commit()
    conn.close()
    print('PATCH SUCCESSFUL: Ultimate WA Metadata fix applied.')
